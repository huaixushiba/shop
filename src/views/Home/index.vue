    ...
    <div class="banner">
      <el-carousel indicator-position="outside"
        ...
        <el-carousel-item v-for="item in banner"
                          :key="item.id">
          <!-- <h3>{{ item }}</h3> -->
          <img v-if="item.picUrl"
               :src="item.picUrl"
               class="img1">
          <img v-if="item.picUrl1"
               :src="item.picUrl1"
               class="img2 a">
          <img v-if="item.picUrl2"
               :src="item.picUrl2"
               class="img3 b">
        </el-carousel-item>
      </el-carousel>
    ...
    <div v-for="item in homeList"
      ...
      <section class="w mt30 clearfix"
        ...
        <m-shelf :title='item.name'>
          ...
          <div slot="content"
               class="hot">
            <MallGoods v-for="panelContent in item.panelContents"
                       :key="panelContent.id"
                       :good='panelContent' />
          </div>
        </m-shelf>
      ...
      <section class="w mt30 clearfix"
        ...
        <m-shelf :title='item.name'>
          <div slot="content"
               class="floors">
            <div class="imgbanner"
                 v-for="panelContent in item.panelContents"
                 :key="panelContent.id"
                 v-if="panelContent.type===2||panelContent.type===3">
              <img :src="panelContent.picUrl"
                   alt="">
            </div>
            <MallGoods v-for="panelContent in item.panelContents"
                       :key="panelContent.id"
  ...
  async created () {
    try {
      ...
      if (data.code === 200) {
        ...
        let banner = result.filter(item => item.name === '轮播图')
        this.banner = banner[0].panelContents
      }
    } catch (error) {
